...
export const allocateTransferMoney = async (
  ...
  try {
    ...
    if (actualInstruction === undefined) {
      ...
      console.log(pvp);
      throw new Error(
        "Actual instruction not found. See the asset (ours), settlement instructions and Pvp"
      );
    }

    const cbdcArray = cbdc
      .filter(
        (c) =>
          c?.payload.asset.id.label ===
            actualInstruction?.payload.asset.id.label &&
          c.payload.asset.earmark === actualInstruction.payload.asset.earmark
      )
      .map((c) => c.contractId);
    await ledger?.exercise(
      SettlementInstruction.SettlementInstruction_AllocateNext,
      actualInstruction.contractId,
      {
