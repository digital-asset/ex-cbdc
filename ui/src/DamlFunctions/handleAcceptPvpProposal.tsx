...
export const handleAcceptPvpProposal = async(setIsLoading: React.Dispatch<React.SetStateAction<boolean>>,
    setIsError: React.Dispatch<React.SetStateAction<boolean>>,
    pvpProposals, ourAssetSettlements, ourBankRoles, BankRole, ledger):Promise<any> => {
    try{
        setIsLoading(true)
        setIsError(false)
        ...
        // @ts-ignore
        const incomingPaymentCashLabel = pvpProposal?.payload.dvp.payments[0].id.label;
        const incomingCb = getPartyId(incomingPaymentCashLabel === Currency.USD ? Central.CentralBank1 : Central.CentralBank2);
        ...
        // @ts-ignore
        const outgoingPaymentCashLabel = pvpProposal.payload.dvp.deliveries[0].id.label;
        const outgoingCb = getPartyId(outgoingPaymentCashLabel === Currency.USD ? Central.CentralBank1 : Central.CentralBank2);
