...
const CentralBankFirstFlow = (props: { displayName: string }) => {
    ...
    const handleCreateIssue =async ()=>{
       try{
           setIsLoading(true)
           setIsError(false);
           await ledger.exercise(CentralBankRole.CreateCbdc, usFRBRoleCId!, { quantity: `${inputValue}000000`, targetAccount:null })
           setIsOpen(false)
    ...
    const centralButtons:{disabled:boolean, label:string, onClick:(e:MouseEvent)=>void, buttonStyle:{}}[] = [
        {
            disabled:false,
            label:"Issue",
            onClick:()=> {
            ...
            buttonStyle:`${styles.buttonStyle}  ${isIssueShown ? styles._openSelect : ''}`
        },
        {
            disabled: false,
            label: 'Transfer',
            onClick: () => {
                setInputValue('')
                if(isIssueShown){setIsIssueShown(false)}
                setIsShow(!isShow)
            },
    ...
    function SET_LIST(buttons, bank) {
        const list = buttons.map(el => (
            <Button
                key={el.label}
                buttonStyle={el.buttonStyle ? el.buttonStyle : styles.buttonStyle} {...el}
            />
        ));
        ...
        if (isShow || isIssueShown) {
            ...
            if (isError) {
                list.push(
                    ...
                    <p className={styles.errorText}>Transfer Failed:<br/>Insufficient Funds</p>
                    </div>)
            }
                ...
                <div className={`${styles.btnTransferContainer} ${styles._inProgress}`} key='Transfer Button'>
                    {isLoading ? <Progress/> : (
                        <Button
                            ...
                            disabled={!isIssueShown?!(inputValue && selectValue):!inputValue}
                        />
                    )}
                </div>
            )
    ...
    function SET_TABLE() {
        ...
        const formatMoney = (money:number) => money===0
            ? "0"
            : `${money/1000000}M ${currency}`

        return [
            getBalance(balances, getPartyId(displayName), formatMoney, BANK_LABELS[displayName]),
            getBalance(balances, getPartyId(Commercial.BankA), formatMoney, BANK_LABELS[Commercial.BankA]),
            getBalance(balances, getPartyId(Commercial.BankB), formatMoney, BANK_LABELS[Commercial.BankB]),
        ];
    }
    ...
    return (
        <div
            ...
            <div className={`${styles.bankTop} ${styles.bankBorder}`}>
                ...
                <h6 className={`${styles.bankText} ${styles._topTitle} ${displayName === Central.CentralBank2 && styles._euTitlePaddings}`}>{title}</h6>
                <Dropdown
                    onClick={() => {
            ...
            <div className={`${styles.bankTriangle} ${displayName === Central.CentralBank2 ? styles._euTrgl : ''}`}/>
            {isError
