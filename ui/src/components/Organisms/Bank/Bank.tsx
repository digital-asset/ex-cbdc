...
import {
    ...
    disabledPvpStatuses,
    ProposePvpModel,
    ProposeStatusType} from "../../../models/ProposePvpModel";
...
const handleDeclinePvp = async(pvpProposals, ledger, DvpProposal) => {
    try{
        const pvpProposal:{contractId:string} = _.first(pvpProposals) || {contractId:''};
        if (pvpProposal === undefined) {
            console.log("No PvP proposal.");
            return;
        }
        await ledger.exercise(DvpProposal.DvpProposal.DvpProposal_Reject, pvpProposal.contractId, {});
    }catch (e){
        console.error(e)
    }
}
...
const Bank: React.FC<CommercialBankData> = (props) => {
    ...
    const [inputBuyCurrency, setInputBuyCurrency] = useState<string>(Currency.USD);
    ...
    const ourBankRoles = useStreamQueries(BankRole, () => [{bank: party}], []).contracts;

    const ourAssetSettlements = useStreamQueries(AssetSettlementRule, () => [{account: {owner: party}}], []).contracts;

    const pvpProposals = useStreamQueries(DvpProposal.DvpProposal).contracts;

    function getBalanceFor(currency) {
        const [normal] = getBalances(assets?.contracts.filter(c => c?.payload.asset.id.label === currency));
        return normal.length ? normal[0].quantity : 0;
    }

    const {pvp, ourAsset, actualInstruction, pvpStatus} =
        getPvpStatus(settlementInstructions, pvpProposals, dvps, PartyId.from(party))

    const convertMoney = (money: number) => (money / 1000000).toString().concat(money === 0 ? "" : "M");

    const isFormShown = !pvpStatus[ProposePvpModel.STATUS]
    ...
    const buttons = [
        {
            disabled: disabledPvpStatuses.some(status => status === pvpStatus[ProposePvpModel.STATUS]),
            label: 'Propose PVP',
            onClick: () => {
                setIsProposeBlockShown(!isProposeBlockShown)
                setIsError(false)
            },
            buttonStyle: `${styles.buttonStyle} ${isFormShown ? styles._openSelect : ''}`
        },
        {
            disabled: pvpStatus[party] || disabledAllocateStatuses.some(status => status === pvpStatus[ProposePvpModel.STATUS]),
            label: 'Allocate',
            onClick: () => {
                allocateTransferMoney(setIsLoading, setIsError, settlementInstructions, cbdc, actualInstruction, ourAsset, pvp, ledger, SettlementInstruction, party)
                setIsOpen(false)
            },
            buttonStyle: `${styles.buttonStyle} ${isFormShown ? styles._openSelect : ''}`
        },
    ...
    function SET_LIST(bank: string) {
        ...
        const list = [
            <Button
                ...
                {...buttons[0]}
            />,
            isLoading && (statusesForProgress.some(status => status === pvpStatus[ProposePvpModel.STATUS])) ?
        ...
        const listData = [
            ...
            />,
            <div className={`${styles.btnTransferContainer} ${isLoading ? styles._inProgress : ''}`}
                    key='Transfer Button'>
                {
                    isLoading ? <Progress key={Math.random()}/> : (
                        <Button
                            key={Math.random()}
                            label='Submit'
        ...
        let approvedData = [
            <p key={Math.random()} className={`${styles.dropDownText} ${styles.dropdownTitleStyle}`}>PVP Proposal:</p>,
            <PvpStatusItem key={Math.random()} title={"Proposer:"} result={getDisplayName(pvpStatus[ProposePvpModel.PROPOSER]!)}/>,
            <PvpStatusItem key={Math.random()} title={"Counterparty:"}
                            result={getDisplayName(pvpStatus[ProposePvpModel.COUNTERPARTY]!)}/>,
            <PvpStatusItem key={Math.random()} title={"Proposer To Buy:"}
                            result={`${pvpStatus[ProposePvpModel.PROPOSER_TO_BUY].amount} Million ${pvpStatus[ProposePvpModel.PROPOSER_TO_BUY].currency}`}/>,
            <PvpStatusItem key={Math.random()} title={"Proposer To Sell:"}
                            result={`${pvpStatus[ProposePvpModel.PROPOSER_TO_SELL].amount} Million ${pvpStatus[ProposePvpModel.PROPOSER_TO_SELL].currency}`}/>,
            <PvpStatusItem
                key={Math.random()}
                title={"Status:"}
        ...
        if (! pvpStatus[ProposePvpModel.PROPOSER]?.equals(getPartyId(bank)) && pvpStatus[ProposePvpModel.STATUS] === ProposeStatusType.AWAITING_RESPONSE) {
            approvedData.push(
                <div key={Math.random()} className={styles.acceptButtons}>{isLoading ?
                    <Progress key={Math.random()}/> :
                    <PvpButtons key={Math.random()} accept={handleAcceptPvp} decline={handleCancelPvp}/>}
                </div>)
        }

        if (isProposeBlockShown) {
            if (!isFormShown) {
                approvedData.forEach(i => list.splice(++spliceNumber, startPosition, i))
            } else {
    ...
    return (
        <div
            ...
            <div className={`${styles.bankTop}`}>
                <div
                    className={`${styles.bankImgContainer} ${displayName === Central.CentralBank2 && styles._eu}`}>
                    <img className={styles.bankImg} src={src} alt="Central Bank"/>
                </div>
                <h6 className={`${styles.bankText} ${styles._topTitle}  ${styles._topTitleCommercial} `}>{title}</h6>
