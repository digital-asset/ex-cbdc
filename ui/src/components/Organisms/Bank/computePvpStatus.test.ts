...
import { PvpStatuses, ProposeStatusType } from "../../../models/ProposePvpModel";
...
it('computePvpStatus smoke', () => {
  ...
  const testInput = {
      ...
      pvpProposals: [{payload: {dvp: {
        deliveries: [ asset ],
        payments: [ asset ],
        masterAgreement: { party1: "BankA", party2: "BankB"},
        buyer: "BankA"
      }}}],
      settlementInstructions: { contracts: [{payload: {asset: asset, steps: [{depositCid: {}}]}}] },
      actualInstruction: {
        payload:  {
          asset:  asset,
          steps:  [ { senderAccount:  {} } ],
  ...
  const {pvp, pvpProposals, settlementInstructions, actualInstruction, bankAllocationDone} = testInput
  const expected = {
      "BankA": null,
      "BankB": null,
  ...
  const actual = computePvpStatus(pvp, pvpProposals, settlementInstructions, actualInstruction, bankAllocationDone)
...
it('getPvpStatusDetails smoke', () => {
  ...
  const settlementInstructions = { contracts: [{payload: {asset: assetB, steps: [{depositCid: {}}]}}] }
  const dvps = [
  ...
  expect(getPvpStatusDetails(settlementInstructions, dvps, PartyId.from("BankA"))).toStrictEqual(expected)
});
