...
export function getPvpStatus(settlementInstructions, pvpProposals, dvps, party: PartyId) {
...
export function getPvpStatusDetails(settlementInstructions, dvps, party: PartyId) {
  ...
  const pvp = handleSortContracts.some(i => _.isNull(i.payload.settledAtTime)) ? _.first(handleSortContracts) : _.last(handleSortContracts)
  const ourAsset = party.asString() === pvp?.payload.buyer ? pvp?.payload.payments[0] : pvp?.payload.deliveries[0]
  const actualInstruction = settlementInstructions.contracts!.find(c =>
  ...
  const bankAllocationDone = actualInstruction?.payload.steps[0].depositCid ? true : false
...
export function computePvpStatus(pvp: CreateEvent<Dvp>,
    ...
    bankAllocationDone: boolean)
{
    ...
    if (pvp && !pvpProposals.length && settlementInstructions.contracts.every(i => i.payload.steps[0].depositCid === null)) {
        const firstContract = pvp.payload
        getPvpTransfers(firstContract, ProposeStatusType.ACCEPTED)
    }
    if (pvp?.payload.status === PvpStatuses.ConfirmAllocation && settlementInstructions.contracts.some(i => i.payload.steps[0].depositCid !== null)) {
        const firstContract = pvp.payload
        getPvpTransfers(firstContract, ProposeStatusType.AWAIT_ALLOCATION)
    }
    if (pvp?.payload.status === PvpStatuses.AllocationsDone && !pvpProposals.length) {
        pvpStatus[ProposePvpModel.STATUS] = ProposeStatusType.SETTLED
    }
    if (bankAllocationDone)
        pvpStatus[actualInstruction?.payload.steps[0].senderAccount.owner || ""] = true
    return pvpStatus;
}
