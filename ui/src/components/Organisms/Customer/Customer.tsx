...
const Customer: React.FC<CustomerProps> = (props) => {
  const { name, containerStyles, withMenu = false } = props;
  ...
  const rentInvoice = useStreamQueries(
    RentInvoice,
    () => [{ renter: party }],
    []
  ).contracts;
  const cbdcs = useStreamQueries(
    AssetDeposit,
    () => [{ account: { owner: party } }],
    []
  ).contracts;
  ...
  const bodyPayRentInvoice = async (): Promise<any> => {
    try {
      ...
      await ledger.exercise(RentInvoice.RentInvoice_Pay, rentInvoiceCid, {
        assetDepositCids: cbdcs.map((i) => i.contractId),
      });
    } catch (e) {
      console.error(e);
      setIsError(true);
  ...
  const { value: balanceUsd } = getBalance(
    balancesUsd,
    PartyId.from(party),
    (num) => `${num} USD`,
    ""
  );
  const { value: balanceUsd_S } = getBalance(
    balancesUsd_S,
    PartyId.from(party),
    (num) => `${num} USD-S`,
    ""
  ...
  return (
    <div className={`${styles.bank} ${containerStyles}`}>
      ...
      <div className={styles.bankList}>
        <p
          className={`${styles.bankText} ${styles.bankListItem}`}
          id="test-alice-balance-normal"
        >
          {balanceUsd}
        </p>
        <p
          className={`${styles.bankText} ${styles.bankListItem}`}
          id="test-alice-balance-stimulus"
        >
          {balanceUsd_S}
        </p>
      </div>
