...
const CentralBank = (props: {
  alice: PartyId;
  landlord: PartyId;
  isCustomer: any;
}) => {
  const { alice, landlord, isCustomer } = props;

  const ledger = useLedger();
  const party = useParty();

  const usFRBRole = useStreamQueries(
    CentralBankRole,
    () => [{ centralBank: party }],
    []
  ).contracts;
  const accountsProvidedByThisCb = useStreamQueries(
    AssetSettlementRule,
    () => [{ account: { provider: party } }],
    []
  ).contracts;
  ...
  const assetDeposits = useStreamQueries(
    AssetDeposit,
    () => [{ account: { provider: party } }],
    []
  ).contracts;
  ...
  const balances = [
    ...
    getBalance(
      ...
      landlord,
      (qty) => `${qty} USD`,
      LABEL[BankCustomer.AlphaProperties]
    ),
  ];

  function getAccountFor(
    rules: readonly CreateEvent<AssetSettlementRule>[],
    partyId: PartyId
  ): CreateEvent<AssetSettlementRule> | undefined {
    return _.first(
      rules.filter((c) => PartyId.from(c.payload.account.owner).equals(partyId))
    );
  }

  // targetPartyDisplayName - renter, in practice probably Alice
  // quantity    - the amount of stimulus CBDC to create
  const bodyForRestrictedStimulus = async (
    targetPartyDisplayName: string
  ): Promise<any> => {
    try {
  ...
  function assertProperRoles(
    cbRole: readonly CreateEvent<CentralBankRole>[],
  ...
  const toggleDropdown = (value: boolean, fc: (item: boolean) => void) => () =>
    fc(value);
