...
const Customer: React.FC<UserProps> = (props) => {
  ...
  const landlordRole = useStreamQueries(
    LandlordRole,
    () => [{ landlord: party }],
    []
  ).contracts;
  const certs = useStreamQueries(
    DistributedEconomicSectorCertificate,
    () => [{ owner: party }],
    []
  ).contracts;
  ...
  function assertProperRoles(
    landlordRoles: readonly CreateEvent<LandlordRole>[],
    cert: CreateEvent<DistributedEconomicSectorCertificate> | undefined
  ) {
    if (landlordRoles.length !== 1) {
      throw new Error(
        "Error, we should have exactly one Landlord Role per landlord."
      );
    }
    if (cert === undefined) {
  ...
  function listForDropdown() {
    ...
    const submitButton = isLoading ? (
      ...
      <Button
        key="button-Recipient"
        label={"Submit"}
        id="test-invoice-submit"
        onClick={() => {
    ...
    if (isShown) {
      list.push(
        <Select
          ...
          selectContainerStyle={styles._selectStyle}
        />,
        <Input
          key="input-Recipient"
