...
const BtnActions: React.FC<BtnActionsProps> = (props) => {
  const { buttons, locationList } = props;
  ...
  const handleLedgerReset = async (): Promise<any> => {
    try {
      setIsLoading(true);
      const demoAdmin =
        _.first(demoAdminRole.contracts)?.payload.demoAdmin || "";
      const payload = { party: demoAdmin };
      await ledger
        .create(ResetImplementation.ResetRequest, payload)
        .catch((e) => {
          console.log(e);
        });
      history.replace("/");
  ...
  return (
    <div className={`${styles.actions}`}>
      {buttons.map((button) => (
        <TextButton
          textBtnIconStyle={`${
            isDropdownShown && button.text === "SECTIONS"
              ? styles._activeDropdown
              : null
          }`}
          key={Math.random()}
          text={button.text}
          ...
          handleAction={
            button.text === "RELOAD" ? handleLedgerReset : handleToggleDropdown
          }
        />
      ))}
      {isDropdownShown && (
        <div className={styles.dropdownContainer}>
          {dropdownList.map((button) => (
            <TextButton
              textBtnContainerStyle={`${styles.dropdownItem} ${
                button.active ? styles._dropdownActiveContainer : null
              }`}
              key={Math.random()}
              textBtnTextStyle={`${
                button.active ? styles._dropdownActiveIcon : null
              }`}
              textBtnIconStyle={styles.dropdownIcon}
              text={button.text}
              icon={
                button.icon ? (button.active ? activePath : button.icon) : null
              }
