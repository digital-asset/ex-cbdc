...
import {
  computePvpStatus,
  getPvpStatusDetails,
} from "../../Organisms/Bank/computePvpStatus";
...
const convertMoney = (money: string) =>
  (+money / 1000000).toString().concat("M");
...
function getAllocateStatus(
  ...
  function updateAllocateStatus(allocateStatus, party: PartyId) {
    ...
    const label: string =
      actualInstruction?.payload.asset.id.label ?? ourAsset?.id.label ?? "";
    const quantity: string =
      actualInstruction?.payload.asset.quantity ?? ourAsset?.quantity ?? "";

    if (actualInstruction || pvp)
      allocateStatus[label] = {
        quantity,
        owner: party,
        isDone: actualInstruction ? bankAllocationDone : true,
      };
  }
...
export function PvpProcessVisualization(props: {
  bankA: PartyId;
  bankB: PartyId;
}) {
  ...
  const allocateStatus = getAllocateStatus(
    settlementInstructions,
    dvps,
    bankA,
    bankB
  );
  const { pvp, actualInstruction, bankAllocationDone } = getPvpStatusDetails(
    settlementInstructions,
    dvps,
    bankA
  );
  ...
  const showBoxes = boxesAllowedStatuses.some(
    (status) => pvpStatus[ProposePvpModel.STATUS] === status
  );
  const showArrows =
      ...
      {showArrows && (
        <div className={styles.damlArrows}>
          ...
          <img
            src={BigRightArrow}
            alt={"#"}
            className={styles.damlRightArrow}
          />
        </div>
      )}
    </>
