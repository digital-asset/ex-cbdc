...
import { LastPaidRentInvoice, RentInvoice } from '@daml.js/landlord-1.0.0/lib/Landlord/Landlord';
...
const [
  ...
  customerCredentials,
  landlordCredentials,
] = [Central.CentralBank1, BankCustomer.Alice, Landlord.Landlord].map((el: string) => computeCredentials(el));
...
function RentInvoiceProcessVisualization() {
  ...
  const [stage, price]: [InvoiceStage, string] =
    rentInvoice.contracts.length
    ? [InvoiceStage.Issued, rentInvoice?.contracts[0].payload.price]
    : lastPaidRentInvoice.contracts.length
      ? [InvoiceStage.Paid, lastPaidRentInvoice?.contracts[0].payload.rentInvoice.price]
      : [InvoiceStage.Nothing, ""]

  function inputData(price): JSX.Element {
    return (
      <div>
        <img src={Certificate} alt="Certificate" className={styles.certificateImg}/>
        {`INVOICE: ${price}`}
      </div>);
  }
...
const Main: React.FC = () => {
  ...
  const leftLogos = () => {
    return (
      <>
        ...
        <img src={ThreePointsLeft} alt={"#"} className={styles.threePointsLeft} />
  ...
  const rightLogos = () => {
    return (<>
      ...
      <img src={ThreePointsRight} className={styles.threePointsRight} alt={"#"} />
  ...
  return (
    <div className={styles.content}>
      ...
      <h1 className={styles.contentTitle}>TRUE INTEROPERABILITY: EXTENSIBILITY</h1>
      <InfoComponent containerStyle={styles.hintContainer}
...
const infoList =[
  {img:FirstPoint, text:"Use the controls for the United States Reserve Bank to transfer Stimulus dollars (any amount) to Alice B. "},
  {img:"", text:"Alice’s USD account balance will update to reflect that transfer."},
  {img:SecondPoint, text:"Use the controls for the United States Reserve Bank to transfer Restricted Stimulus dollars in any amount (which in this example, may only be used for rent payments) to Alice B. "},
  {img:"", text:"Alice’s USD-S account balance will update to reflect that transfer."},
  {img:ThirdPoint, text:"Next, use the controls for Alice’s landlord, Alpha Properties, to send her an invoice for a monthly rent payment. "},
  {img:FourthPoint, text:"Finally, use the controls for Alice to respond to the invoice by paying her rent with her USD-S balance. "},
  {img:"", text:"Afterwards, the balance of Alpha Properties will show an increase in regular USD.  "},
]
const AdditionalHints =()=><p className={styles.textHints}>During the rental payment, Alice’s USD-S balance was prioritized; if that balance was too low to cover her rent, then the remainder was paid from Alice’s regular USD balance.</p>
